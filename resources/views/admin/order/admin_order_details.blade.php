                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if ($order->status == 'Pending')
                                                    <a href="{{ route('admin.orders.pening_to_confirm', ['order' => $order]) }}"
                                                        class="btn btn-block btn-success" id="confirmOrder">Confirm
                                                        Order</a>
                                                @elseif ($order->status == 'confirm')
                                                    <a href="{{ route('admin.orders.confirm_to_processing', ['order' => $order]) }}"
                                                        class="btn btn-block btn-success" id="processingOrder">Processing
                                                        Order</a>
                                                @elseif ($order->status == 'processing')
                                                    <a href="{{ route('admin.orders.processing_to_deliverd', ['order' => $order]) }}"
                                ...
                                <tbody>
                                    ...
                                    @foreach ($orderItem as $item)
                                        <tr>
                                            ...
                                            {{-- Coupon --}}
                                            @php
                                                $product_has_coupon = 0;
                                                if ($order->coupon) {
                                                    if ($order->coupon->client_id == $item->client_id) {
                                                        $product_has_coupon = 1;
                                                    }
                                                }
                                            @endphp
                                            ...
                                            <td class="col-md-2">
                                                <label>
                                                    {{ $item->price }} <br> Total = $ {{ $item->total_price }}
                                                </label>
                                            </td>
                                        </tr>
                                    @endforeach
